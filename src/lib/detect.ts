...
export async function getLocation(ip: string = '', headers: Headers, hasPayloadIP: boolean) {
  ...
  if (!hasPayloadIP && !process.env.SKIP_LOCATION_HEADERS) {
    ...
    if (headers.get('cf-ipcountry')) {
      ...
      const lat = decodeHeader(headers.get('cf-iplatitude'));
      const lng = decodeHeader(headers.get('cf-iplongitude'));
    ...
    if (headers.get('x-vercel-ip-country')) {
      ...
      const lat = decodeHeader(headers.get('x-vercel-ip-latitude'));
      const lng = decodeHeader(headers.get('x-vercel-ip-longitude'));
